...
while true; do
	...
	ok|good) echo "okie dokie artichokie"; break;;
...
while true; do
	...
	morning)
		case "$mood" in  
		happy|alright|ok|good) echo "It is a beautiful morning, $name!"; break;;
...
case "$time" in
	afternoon|noon)
		...
		alright|ok|good) echo "Is it lunch time yet?"; break;;
...
case "$time" in 
	night|evening)
		...
		alright|ok|good) echo "Have a good evening $name"; break;;
...
sleep .3s
	...
	redo=${redo,,}
	case $mood in
		fuck*you|fuck*off) echo "Cunt"; sleep 5s; break;;
	esac
	case $redo in
		yes) ;;
